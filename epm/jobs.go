...
var NoTxerErr = fmt.Errorf("Txer is nil")
...
func (e *EPM) Assert(args []string) error {
	...
	got = strings.ToLower(common.StripZeros(common.StripHex(got)))
	expected = strings.ToLower(common.StripZeros(common.StripHex(expected)))
	if got != expected {
...
func (e *EPM) Include(args []string) error {
	...
	for i := 0; i < len(args)/2; i++ {
		...
		absIncludePath := path.Join(common.GoPath, "src", includePath)
		if _, err := os.Stat(absIncludePath); err != nil {
			...
			cur, _ := os.Getwd()
			os.Chdir(path.Join(common.GoPath, "src"))
