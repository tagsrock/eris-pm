...
then
  ...
  lllc-server --no-ssl --unsecure-port ${UNSECURE_PORT:=9099}
else
  if [ -z $SSL_ONLY ]
  then
    echo "Starting server using HTTP+HTTPS"
    lllc-server --unsecure-port ${UNSECURE_PORT:=9099} --secure-port ${SECURE_PORT:=9098} --key /data/key.key --cert /data/cert.crt
  else
    echo "Starting server --secure-only"
    lllc-server --secure-only --secure-port ${SECURE_PORT:=9098} --key /data/key.key --cert /data/cert.crt
  fi
fi
